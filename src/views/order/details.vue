		...
		<div class="content " style="margin-top:0;" v-show="tabindex == 1">
			<div class="pd-l  bdb">
				<el-table :data="tableData2" style="width: 100%"
					...
					<el-table-column  label="备注">{{ postInfo.specialNote }}</el-table-column>
					<el-table-column  label="操作"><el-button @click="changeRemarkDialog">修改备注</el-button></el-table-column>
			...
			<!-- 修改备注dialog -->
			<el-dialog v-model="diaflagRemark" title="修改备注信息">
			<div class="editnum-dialog">
				<el-form>
					<el-form-item label="备注：" prop="specialNote">
						<el-input type="textarea" v-model="ruleForm1.specialNote" placeholder="请输入备注"/>
					</el-form-item>
				</el-form>
			...
			<div class="dialog-btn-div ">
				<el-button class="btn" @click="diaflagRemark = false">
					取消
				</el-button>
				<el-button class="btn query" @click="onlyChangeRemark()">
					确认
				</el-button>
			</div>
		</el-dialog>
			...
			<div class="editnum-dialog">
				<el-form ref="ruleFormRef" label-width="100px" :rules="rules" :model="ruleForm" style="max-width: 460px">
					<el-form-item label="物流公司：" prop="logistics">
						<el-select v-model="ruleForm.logistics" @change="tongcheng" placeholder="请选择物流公司"  style="width: 240px">
					...
					<el-form-item label="备注：" prop="specialNote">
						<el-input type="textarea" v-model="ruleForm.specialNote" placeholder="请输入备注"/>
					</el-form-item>
				</el-form>
			</div>
			<div class="dialog-btn-div ">
		...
		<el-dialog v-model="diaflagModify" title="修改物流信息">
			<div class="editnum-dialog">
				<el-form ref="ruleFormRef1" label-width="100px" :rules="rules" :model="ruleForm1" style="max-width: 460px">
					<el-form-item label="物流公司：" prop="logistics">
						<el-select v-model="ruleForm1.logistics" @change="tongcheng" placeholder="请选择物流公司" style="width: 240px">
							<el-option v-for="item in options" :key="item.label" :label="item.value" :value="item.label" />
						</el-select>
					</el-form-item>
					<el-form-item label="物流单号：" prop="logisticsId">
...
import {getDetailData, updateLogisticsInfo, queryLogisticsInfo, logisticsModify,logisticsRemarkModify} from "@/api/oms/order";
  ...
  getSysDictInfo('1008').then((res) => {
	...
	getDetailData(req).then((res) => {
		...
		if (res.data.resultBody.busiResp) {
			...
			// info1.key ="订单号"
			state.orderInfo.orderId = res.data.resultBody.busiResp.busiDataResp[0].orderId;

			// info1.key ="下单时间"
			state.orderInfo.createTime = res.data.resultBody.busiResp.busiDataResp[0].createTime;


			//list2
			// info1.key ="卖家名称"
			state.supplierInfo.supplierName = res.data.resultBody.busiResp.busiDataResp[0].shipperName;

			// info1.key ="商家编号"
			state.supplierInfo.supplierId = res.data.resultBody.busiResp.busiDataResp[0].shipperId;

			// info1.key ="买家备注"
			state.supplierInfo.remark = res.data.resultBody.busiResp.busiDataResp[0].remark;
			...
			state.buyerInfo.buyerName = str2.substring(0, 3) + "****" + str2.substring(str2.length - 4);
			// info1.key ="买家身份"
			...
			state.buyerInfo.buyerMoblie = str3.substring(0, 3) + "****" + str3.substring(str3.length - 4);
			// info1.key ="地址"
			state.buyerInfo.buyerAddr = res.data.resultBody.busiResp.busiDataResp[0].province + res.data.resultBody.busiResp.busiDataResp[0].city + res.data.resultBody.busiResp.busiDataResp[0].country + res.data.resultBody.busiResp.busiDataResp[0].detailAddr;
			//list4
			// info1.key ="合计金额"
			state.chargeInfo.mount = res.data.resultBody.busiResp.busiDataResp[0].billCharge;
			state.charge = res.data.resultBody.busiResp.busiDataResp[0].charge;
			// info1.key ="配送费用"
			state.chargeInfo.postFee = res.data.resultBody.busiResp.busiDataResp[0].freight;
			//info1.key ="优惠金额"
			// state.chargeInfo[2].value = res.data.resultBody.busiResp.busiDataResp[0].coupon;
			//list5
			// info1.key ="收货人"
			state.checkInfo.receiver = res.data.resultBody.busiResp.busiDataResp[0].postInfo.receiver;
			// info1.key ="联系方式"
			state.checkInfo.mobile = res.data.resultBody.busiResp.busiDataResp[0].postInfo.mobile;
			// info1.key ="确认收货时间"
			state.checkInfo.receivetime = res.data.resultBody.busiResp.busiDataResp[0].receiptTime;
			// info1.key ="收货地址"
			state.checkInfo.addr = res.data.resultBody.busiResp.busiDataResp[0].postInfo.province + res.data.resultBody.busiResp.busiDataResp[0].postInfo.city + res.data.resultBody.busiResp.busiDataResp[0].postInfo.country + res.data.resultBody.busiResp.busiDataResp[0].postInfo.detailAddr;
			//list6
			// info1.key ="是否需要发票"
			...
			// list7
			// info1.key ="支付时间"
			state.purchase.type = res.data.resultBody.busiResp.busiDataResp[0].payType == '' && res.data.resultBody.busiResp.busiDataResp[0].status != '2' ? '在线支付' : res.data.resultBody.busiResp.busiDataResp[0].payType;

			state.purchase.time = res.data.resultBody.busiResp.busiDataResp[0].payTime;
			state.purchase.tradeId = res.data.resultBody.busiResp.busiDataResp[0].tradeId;
			state.purchase.closeTime = res.data.resultBody.busiResp.busiDataResp[0].closeTime;
			...
			state.tableData1.forEach(x => {
				...
				state.chargeInfo.cheapFee = state.chargeInfo.cheapFee + x.coupon
...
// const changeRemark = async (formEl: FormInstance | undefined) => {
// 	if (!formEl) return
// 	await formEl.validate((valid) => {
// 		if (valid) {
// 			modify()
...
function tongcheng(){
	...
	if (ruleForm1.logistics=="tongchengpeisong"){
		ruleForm1.logisticsId="同城配送"
	}else{
		ruleForm1.logisticsId=""
	}
	if (ruleForm.logistics=="tongchengpeisong"){
		ruleForm.logisticsId="同城配送"
	}else{
		ruleForm.logisticsId=""
	}
}
...
function send() {
	...
	arg.value.specialNote = ruleForm.specialNote;
	
	arg.value.orderId = route.query.id;
	// if(arg.value.logistics =="tongchengpeisong"){
	// 	arg.value.logisticsId="同城配送"
	// 	ruleForm1.logisticsId="同城配送"
	// }
	updateLogisticsInfo(arg.value).then(res => {
		console.log(res.data)
		state.diaflag = false
		ruleForm1.logistics = ruleForm.logistics;
		ruleForm1.logisticsId = ruleForm.logisticsId;
		ruleForm1.deliveryTime = ruleForm.deliveryTime;
		ruleForm1.specialNote = ruleForm.specialNote;
...
function modify() {
	const arg = {
		data: {
			...
			specialNote:ruleForm1.specialNote
		}
	}
	// if(arg.data.logistics == "tongchengpeisong"){
	// 	arg.data.logisticsId="同城配送"
	// 	ruleForm1.logisticsId="同城配送"
	// }
...
function changeRemarkDialog(){
	state.diaflagRemark = true;
}
function onlyChangeRemark(){
	...
	logisticsRemarkModify(arg).then(_ => {
		state.diaflagRemark = false;
		loadData();
	})
}
